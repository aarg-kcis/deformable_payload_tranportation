...
def generate_bot_paths(leftBotAngles, rightBotAngles):
...
#	long_path = np.loadtxt('./allPaths/long_path.txt')
	...
	for i in range(len(leftBotAngles)):
		left_dists.append(mfm.left_bot_dist_generator(long_path, leftBotAngles[i]))
	right_dists = []
	for i in range(len(rightBotAngles)):
		right_dists.append(mfm.right_bot_dist_generator(long_path, rightBotAngles[i]))
	
	leftDist = left_dists[0]
	for i in range(1, len(left_dists)):
		leftDist = np.minimum(leftDist, left_dists[i])
	rightDist = right_dists[0]
	for i in range(1, len(right_dists)):
		rightDist = np.minimum(rightDist, right_dists[i])

	leftBotPaths = []
	for i in range(len(left_dists)):
#		leftDist = left_dists[i]
		leftBotPaths.append(mfm.bot_path_generator(long_path, np.array(leftDist), leftBotAngles[i]))

	rightBotPaths = []
	for i in range(len(right_dists)):
#		rightDist = right_dists[i]
		rightBotPaths.append(mfm.bot_path_generator(long_path, np.array(rightDist), rightBotAngles[i]))

#	cx, cy = long_path.T
#	lx, ly = leftBotPaths[1].T
#	plt.plot(cx, cy)
#	plt.plot(lx, ly)
#	plt.show()
	return long_path, leftBotPaths, rightBotPaths
