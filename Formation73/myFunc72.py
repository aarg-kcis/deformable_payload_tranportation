...
bound = (0.0, 20.0) #Basically the step size
start_pt = (0,0)
goal_pt = (20,20)
circularObstacles = [(16,5,3), (5,16,3)]
bot_rad = 0.1
idealDistOfBotFromCentr = 5.0
omplclearance = 4.5
minDistOfClearance =3.5# bot_rad*2  + bot_rad #Change it for clearance during path planning
...
def myClearance(x, y):
	...
	for obstacles in circularObstacles:
		val = sqrt((x-obstacles[0])**2 + (y-obstacles[1])**2) - obstacles[2] - omplclearance
		if val > 0:
			success = True
		else:
			success = False
...
def right_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		elif final_dist > minDistOfClearance and final_dist <= idealDistOfBotFromCentr :
			bot2_dist = np.append(bot2_dist, [((final_dist-minDistOfClearance)/2 + (minDistOfClearance - bot_rad) )], axis = 0) #Subhasis: Changed here
...
#	np.savetxt('bot2_dist_o.txt', bot2_dist)
	...
	bot2_dist = scipy.ndimage.minimum_filter1d(bot2_dist,size=150)
	bot2_dist = scipy_gaussian(bot2_dist,sigma=30)
...
def left_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		if final_dist >  idealDistOfBotFromCentr:
			...
			bot1_dist = np.append(bot1_dist, [((final_dist-minDistOfClearance)/2 + (minDistOfClearance - bot_rad) )], axis = 0) #Subhasis: Changed here
		else:
	...
	bot1_dist = scipy.ndimage.minimum_filter1d(bot1_dist,size=150)
	bot1_dist = scipy_gaussian(bot1_dist,sigma=30)
