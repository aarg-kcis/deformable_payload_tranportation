...
start_pt = (0,0)
goal_pt = (10,10)
circularObstacles = [(3,1,1), (3,4,1), (8,2,2), (3,8,2), (6.3, 6.0,  0.5), (9,7,1), (6.5, 7.5,  0.5) ,(8, 5.5,  0.5)]
bot_rad = 0.1
idealDistOfBotFromCentr = 0.8 
minDistOfClearance = bot_rad*np.sqrt(2)  + bot_rad #Change it for clearance during path planning
...
def right_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		elif final_dist > np.sqrt(2)*bot_rad+bot_rad and final_dist <= idealDistOfBotFromCentr :
			bot2_dist = np.append(bot2_dist, [(((final_dist-2*bot_rad)/2 + bot_rad) )], axis = 0) #Subhasis: Changed here
		else:
	...
	bot2_dist = scipy_gaussian(bot2_dist,sigma=5)
	bot2_dist = np.minimum(bot2_dist, bot2_dist_initial)
	bot2_dist = scipy_gaussian(bot2_dist,sigma=1)
...
def left_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		if final_dist >  idealDistOfBotFromCentr:
			...
			bot1_dist = np.append(bot1_dist, [(((final_dist-2*bot_rad)/2 + bot_rad)  )], axis = 0)
		else:
	...
	bot1_dist = scipy_gaussian(bot1_dist,sigma=5)
	bot1_dist = np.minimum(bot1_dist, bot1_dist_initial)
	bot1_dist = scipy_gaussian(bot1_dist,sigma=1)
	return bot1_dist
