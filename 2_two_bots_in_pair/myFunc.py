...
def right_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		if final_dist >  idealDistOfBotFromCentr:
			bot2_path = np.append(bot2_path, [[long_path[i][0] + ( idealDistOfBotFromCentr-bot_rad) * np.cos(bot2_theta), long_path[i][1] + ( idealDistOfBotFromCentr-bot_rad) * np.sin(bot2_theta)]], axis = 0)
			bot2_dist = np.append(bot2_dist, [( idealDistOfBotFromCentr-bot_rad) / abs(np.sin(bot_angle) )], axis = 0)
		elif final_dist > 2*bot_rad and final_dist <= idealDistOfBotFromCentr :
			...
			bot2_dist = np.append(bot2_dist, [(((final_dist-2*bot_rad)/2 + bot_rad) / abs(np.sin(bot_angle)))], axis = 0) #Subhasis: Changed here
		else:
			bot2_dist = np.append(bot2_dist, [((final_dist-bot_rad) / abs(np.sin(bot_angle)))], axis = 0) #Subhasis: Changed here
			print final_dist
...
def left_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		if final_dist >  idealDistOfBotFromCentr:
			bot1_path = np.append(bot1_path, [[long_path[i][0] + ( idealDistOfBotFromCentr-bot_rad) * np.cos(bot1_theta), long_path[i][1] + ( idealDistOfBotFromCentr-bot_rad) * np.sin(bot1_theta)]], axis = 0)
			bot1_dist = np.append(bot1_dist, [( idealDistOfBotFromCentr-bot_rad) / abs(np.sin(bot_angle))], axis = 0)
		elif final_dist <= idealDistOfBotFromCentr and final_dist > 2*bot_rad:
			...
			bot1_dist = np.append(bot1_dist, [(((final_dist-2*bot_rad)/2 + bot_rad)  / abs(np.sin(bot_angle)))], axis = 0)
		else:
