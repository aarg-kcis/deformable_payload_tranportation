...
minDistOfClearance = np.sqrt(2)* bot_rad + bot_rad #Change it for clearance during path planning
...
def right_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		if final_dist >  idealDistOfBotFromCentr:
			...
			bot2_dist = np.append(bot2_dist, [(((final_dist-2*bot_rad)/2 + bot_rad*np.sqrt(2)) )], axis = 0) #Subhasis: Changed here
		else:
...
def left_bot_dist_generator(long_path, bot_angle):
	...
	#for i in range(40, 51):
		...
		if final_dist >  idealDistOfBotFromCentr:
			...
			bot1_dist = np.append(bot1_dist, [(((final_dist-2*bot_rad)/2 + bot_rad*np.sqrt(2))  )], axis = 0)
		else:
