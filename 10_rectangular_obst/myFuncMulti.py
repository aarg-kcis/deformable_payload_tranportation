...
start_pt = (0,0)
goal_pt = (10,10)
circularObstacles = [(3,1,1), (3,4,1), (8,2,2), (3,8,2), (6.0, 6.0,  0.5), (9,7,1), (6.5, 7.5,  0.5) ,(8, 5.5,  0.5)]
leftRectangularObstacles = [(4, 0.5, 6,0.5)]
rightRectangularObstacles = [(2, 0.5, 4,0.5)]
bot_rad = 0.1
...
def intersectWithLine(line1, line2):
	x = 0
	y = 0	
...
def right_bot_dist_generator(long_path, bot_angle):
	...
	for i in range(1, long_path.shape[0]):
		...
		current_dist = 10
		for obstacles in rightRectangularObstacles:
			line1 = LineString([(obstacles[0], obstacles[1]), (obstacles[2], obstacles[3])])
			line2 = LineString([(long_path[i][0], long_path[i][1]), (long_path[i][0] + 10 * np.cos(bot_angle), long_path[i][1] + 10 * np.sin(bot_angle))])
			a = line1.intersection(line2)
			if not a.is_empty:
				current_dist = distance_between_points(a.x, a.y, long_path[i][0], long_path[i][1]) 
...
def left_bot_dist_generator(long_path, bot_angle):
	...
	for i in range(1, long_path.shape[0]):
		final_dist = 10 # Some max value from which distance to intersection with obstacle has to be less.
		current_dist = 10
		for obstacles in leftRectangularObstacles:
			line1 = LineString([(obstacles[0], obstacles[1]), (obstacles[2], obstacles[3])])
			line2 = LineString([(long_path[i][0], long_path[i][1]), (long_path[i][0] + 10 * np.cos(bot_angle), long_path[i][1] + 10 * np.sin(bot_angle))])
			a = line1.intersection(line2)
			if not a.is_empty:
	...
	np.savetxt('leftBot_dist_o.txt', bot1_dist)
	...
	np.savetxt('leftBot_dist.txt', bot1_dist)
	#bot1_dist = scipy_gaussian(bot1_dist,sigma=2)
	return bot1_dist, bot1_dist_initial
